...
CompanyProfile::const_iterator::const_iterator(COMPANY_PROFILE_ENTRY_DEQUE_ITER iterator) : iter(iterator){}

CompanyProfile::const_iterator CompanyProfile::const_iterator::operator++()
{
	++iter;
	return *this;
}

bool CompanyProfile::const_iterator::operator==(const const_iterator& another)
{
	if (this == &another)
		return true;
...
const PSTRING_DEQUE CompanyProfile::const_iterator::operator->()
{
	return ((PCOMPANY_PROFILE_ENTRY)(*iter))->profile_element_deque;
}

const STRING_DEQUE& CompanyProfile::const_iterator::operator*()
{
	return *((PCOMPANY_PROFILE_ENTRY)(*iter))->profile_element_deque;
}
...
CompanyProfile* CompanyProfile::instance = NULL;

CompanyProfile* CompanyProfile::get_instance()
{
	if (instance == NULL)
	{
		...
		if (instance == NULL)
		{
			...
			{
				char errmsg[256];
				snprintf(errmsg, 256, "Fail to initialize the CompanyProfile object , due to: %s", get_ret_description(ret));
...
unsigned short CompanyProfile::initialize()
{
...
unsigned short CompanyProfile::parse_company_profile_conf()
{
...
unsigned short CompanyProfile::parse_company_group_conf()
{
...
int CompanyProfile::add_ref()
{
	__sync_fetch_and_add(&ref_cnt, 1);
	return ref_cnt;
}

int CompanyProfile::release()
{
...
void CompanyProfile::show_traverse_result(CompanyProfile* company_profile, bool company_group_mode)
{
	...
	{
		CompanyProfile::const_iterator iter = company_profile->begin();
...
int CompanyProfile::get_company_group_size()const{return company_group_size;}

std::string CompanyProfile::get_company_group_description(int index)const
{
...
const PSTRING_DEQUE CompanyProfile::get_company_number_list_in_group(int index)
{
	if (index < 0 || index >= company_group_size)
		throw std::invalid_argument("index is Out Of Range");
	...
	static pthread_mutex_t mtx1 = PTHREAD_MUTEX_INITIALIZER; // To avoid dead lock with 'mtx'
	if (!deque_init)
	{
		pthread_mutex_lock(&mtx);
		if (!deque_init)
	...
	if (company_number_list_in_group_vector[index].empty())
	{
		...
		if (company_number_list_in_group_vector[index].empty())
		{
			CompanyProfile::const_iterator iter = group_begin(index);
...
const PSTRING_DEQUE CompanyProfile::lookup_company_profile(string company_number)const
{
...
string CompanyProfile::lookup_company_listing_date(string company_number)const
{
	return (*lookup_company_profile(company_number))[COMPANY_PROFILE_ENTRY_FIELD_INDEX_LISTING_DATE];
}

string CompanyProfile::lookup_company_group_name(string company_number)const
{
	return (*lookup_company_profile(company_number))[COMPANY_PROFILE_ENTRY_FIELD_INDEX_GROUP_NAME];
}

string CompanyProfile::lookup_company_group_number(string company_number)const
{
	return (*lookup_company_profile(company_number))[COMPANY_PROFILE_ENTRY_FIELD_INDEX_GROUP_NUMBER];
}

int CompanyProfile::get_company_group_number(string company_number)const
{
...
bool CompanyProfile::is_company_exist(std::string company_number)const
{
	try
	{
...
unsigned short CompanyProfile::generate_company_profile_sorted_deque()
{
	if (company_profile_sorted_deque != NULL)
	...
	// assert(company_profile_sorted_deque != NULL && "company_profile_sorted_deque should NOT be NULL");
	for(COMPANY_PROFILE_MAP::iterator iter = company_profile_map.begin() ; iter != company_profile_map.end() ; iter++)
	{
		PCOMPANY_PROFILE_ENTRY company_profile_entry = (PCOMPANY_PROFILE_ENTRY)iter->second;
...
unsigned short CompanyProfile::generate_company_group_profile_sorted_deque()
{
	if (company_group_profile_sorted_deque != NULL)
		return RET_FAILURE_INCORRECT_OPERATION;
	...
	// assert(company_group_profile_sorted_deque != NULL && "company_group_profile_sorted_deque should NOT be NULL");
	for (int i = 0 ; i < company_group_size ; i++)
	{
...
void CompanyProfile::init_company_profile_sorted_deque()
{
	if (company_profile_sorted_deque == NULL)
	{
		pthread_mutex_lock(&mtx);
...
void CompanyProfile::init_company_group_profile_sorted_deque()
{
	if (company_group_profile_sorted_deque == NULL)
	{
		pthread_mutex_lock(&mtx);
...
CompanyProfile::const_iterator CompanyProfile::begin() 
{
...
CompanyProfile::const_iterator CompanyProfile::end() 
{
	if (company_profile_sorted_deque == NULL) init_company_profile_sorted_deque();
	return const_iterator(company_profile_sorted_deque->end());
}

CompanyProfile::const_iterator CompanyProfile::group_begin(int index) 
{
	if (index < 0 || index >= company_group_size)
		throw std::invalid_argument("index is Out Of Range");
...
CompanyProfile::const_iterator CompanyProfile::group_end(int index) 
{
	if (index < 0 || index >= company_group_size)
		throw std::invalid_argument("index is Out Of Range");
	if (company_group_profile_sorted_deque == NULL) init_company_group_profile_sorted_deque();
