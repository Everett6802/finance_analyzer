...
QuerySet::QuerySet(FinanceDataType cur_finance_data_type) :
	finance_data_type(cur_finance_data_type),
	add_done(false),
	finance_analysis_mode(FinanceAnalysis_None),
	method_index_set(NULL)
{
	IMPLEMENT_MSG_DUMPER()
	// FINANCE_DATA_FIELD_TYPE_LIST = get_finance_data_field_type_list(finance_data_type);
	// FINANCE_DATA_FIELD_AMOUNT_LIST = get_finance_data_field_amount_list(finance_data_type);
	DEFINE_FINANCE_DATA_FIELD_TYPE_LIST(finance_data_type)
	DEFINE_FINANCE_DATA_FIELD_AMOUNT_LIST(finance_data_type)
}
	...
	method_index_set(NULL)
{
	...
	finance_data_type = another_query_set.finance_data_type;
	finance_analysis_mode = another_query_set.finance_analysis_mode;
	// FINANCE_DATA_FIELD_TYPE_LIST = get_finance_data_field_type_list(finance_data_type);
	// FINANCE_DATA_FIELD_AMOUNT_LIST = get_finance_data_field_amount_list(finance_data_type);
	DEFINE_FINANCE_DATA_FIELD_TYPE_LIST(finance_data_type)
	DEFINE_FINANCE_DATA_FIELD_AMOUNT_LIST(finance_data_type)
	...
	{
		int another_method_index = (int)iter->first;
...
// Initialize a list of keeping track of the field index in certain a source type
		unsigned short ret = init_source_field_query_map_element(another_method_index);
		if(CHECK_FAILURE(ret))
		{
			const int ERRMSG_SIZE = 256;
			char errmsg[ERRMSG_SIZE];
			snprintf(errmsg, ERRMSG_SIZE, "init_source_field_query_map_element(%d) fails, due to: %s", another_method_index, get_ret_description(ret));
		...
		while (iter_field != another_field_deque->end())
		{
			...
			source_field_query_map[another_method_index]->push_back(another_field_index);
			iter_field++;
		}
		iter++;
	}
...
unsigned short QuerySet::init_method_index_set()
{
	method_index_set = new INT_SET();
	if (method_index_set == NULL)
	{
		WRITE_ERROR("Fail to allocate memory: method_index_set");
		return RET_FAILURE_INSUFFICIENT_MEMORY;		
	}
...
unsigned short QuerySet::init_source_field_query_map_element(int method_index)
{
	if (source_field_query_map[method_index] == NULL)
	{
		...
		source_field_query_map[method_index] = int_queue;
	}
	return RET_SUCCESS;
}

FinanceDataType QuerySet::get_data_type()const{return finance_data_type;}

unsigned short QuerySet::add_query(int method_index, int field_index)
{
	if (add_done)
	{
		WRITE_ERROR("the add_done flag is NOT true");
...
// Check if the index is out of range
	// if(!check_method_index_in_range(method_index))
	// {
	// 	WRITE_FORMAT_ERROR("method_index[%d] is out of range in QuerySet", method_index);
	// 	return RET_FAILURE_INVALID_ARGUMENT;
	// }
// Initialize a list of keeping track of the field index in certain a source type
	unsigned short ret = init_source_field_query_map_element(method_index);
	if(CHECK_FAILURE(ret))
		return ret;
	assert(source_field_query_map[method_index] != NULL && "source_field_query_map[method_index] should NOT be NULL");

	if(!check_field_index_in_range(method_index, field_index, finance_data_type))
	{
...
// Add the index
	if (field_index == -1)
		source_field_query_map[method_index]->clear();
	source_field_query_map[method_index]->push_back(field_index);
	return RET_SUCCESS;
}

unsigned short QuerySet::add_query_list(int method_index, const PINT_DEQUE field_index_deque)
{
...
// Check if the index is out of range
	// if(!check_method_index_in_range(method_index))
	// {
	// 	WRITE_ERROR("method_index is out of range in QuerySet");
	// 	return RET_FAILURE_INVALID_ARGUMENT;
	// }
// Initialize a list of keeping track of the field index in certain a source type
	unsigned short ret = init_source_field_query_map_element(method_index);
...
//  Caution: The iterator is const, can NOT be altered
		...
		// }
		int method_index = iter.get_first();
// Check if method_index is in range
		if (!check_method_index_in_range(method_index, cur_finance_analysis_mode))
		{
			WRITE_FORMAT_ERROR("method_index[%d] is out of range in QuerySet", method_index);
			return RET_FAILURE_INVALID_ARGUMENT;
		}
		assert(source_field_query_map[method_index] != NULL && "source_field_query_map[method_index] should NOT be NULL");
		PINT_DEQUE field_deque = source_field_query_map[method_index];
		if ((*field_deque)[0] == -1)
		{
			field_deque->clear();
			for (int field_index = 1 ; field_index < FINANCE_DATA_FIELD_AMOUNT_LIST[method_index] ; field_index++) // Caution: Don't include the "date" field
				field_deque->push_back(field_index);
		}
		else
		{
// Check if field_index of method_index is in range
			...
			while (iter_field != field_deque->end())
			{
				...
				if (!check_field_index_in_range(method_index, field_index, finance_data_type))
				{
					WRITE_FORMAT_ERROR("field_index[%d] of method_index[%d] is out of range in QuerySet", field_index, method_index);
					return RET_FAILURE_INVALID_ARGUMENT;
				}
				iter_field++;		
			}
...
// 	{
// 		assert(source_field_query_map[i] != NULL && "source_field_query_map[method_index] should NOT be NULL");
// 		if (source_field_query_map[i]->empty())
// 			continue;
// //		WRITE_FORMAT_DEBUG("Transform the query data[method_index: %d]", i);
...
unsigned short QuerySet::get_query_sub_set(int method_index, QuerySet** query_sub_set)const
{
...
// Can't call map::operator[] in const member function, since std doesn't provide the const version of operator[]
	// const PINT_DEQUE field_index_deque = (const PINT_DEQUE)source_field_query_map[method_index];
	INT_INT_DEQUE_MAP_CONST_ITER iter = source_field_query_map.find(method_index);
	assert(iter != source_field_query_map.end() && "source_field_query_map[method_index] should NOT be NULL");
	...
	unsigned short ret = query_sub_set_tmp->add_query_list(method_index, field_index_deque);
	if (CHECK_FAILURE(ret))
	{
...
const PINT_SET QuerySet::get_method_index_set()
{
	if (method_index_set == NULL)
	{
		unsigned short ret = init_method_index_set();
		if (CHECK_FAILURE(ret))
			throw bad_alloc();
	}
	return method_index_set;
}

const INT_DEQUE& QuerySet::operator[](int method_index)
{
	if (source_field_query_map.find(method_index) == source_field_query_map.end())
	{
		static char errmsg[64];
		snprintf(errmsg, 64, "The index[%d] is NOT found", method_index);
		WRITE_ERROR(errmsg);
		throw invalid_argument(errmsg);
	}
	assert(source_field_query_map[method_index] != NULL && "source_field_query_map[method_index] should NOT be NULL");
	return *source_field_query_map[method_index];
}
	...
	if (whole_company_number_in_group_map == NULL)
	{
		...
		for (int i = 0 ; i < company_group_size ; i++)
		{
			...
			if (company_number_deque == NULL)
				throw bad_alloc();
			CompanyProfile::const_iterator iter = company_profile->group_begin(i);
...
void ResultSet::get_metadata_string(const ResultSet& result_set, string &result_set_metadata_string)
{
	...
	while (iter != data_set_mapping.end())
	{
		...
		unsigned short method_index = get_upper_subindex(key);
		unsigned short field_index = get_lower_subindex(key);
		switch(result_set.FINANCE_DATA_FIELD_TYPE_LIST[method_index][field_index])
		{
		case FinanceField_INT:
		{
...
void ResultSet::get_string(const ResultSet& result_set, string &result_set_string)
{
	...
	for (int i = 0 ; i < result_set.date_data_size ; i++)
	{
		...
		while (iter != data_set_mapping.end())
		{
			...
			case FinanceField_INT:
			{
				snprintf(buf, BUF_SIZE, "| %9d ", result_set.get_int_array_element(method_index, field_index, i));
			}
			break;
			case FinanceField_LONG:
			{
				snprintf(buf, BUF_SIZE, "| %12ld ", result_set.get_long_array_element(method_index, field_index, i));
			}
			break;
			case FinanceField_FLOAT:
			{
				snprintf(buf, BUF_SIZE, "| %8.2f ", result_set.get_float_array_element(method_index, field_index, i));
			}
			break;
			case FinanceField_DATE:
			{
...
ResultSet::ResultSet(FinanceDataType cur_finance_data_type) :
	finance_data_type(cur_finance_data_type),
	check_date_data_mode(false),
	data_set_mapping_size(0)
{
...
FinanceDataType ResultSet::get_data_type()const{return finance_data_type;}
...
unsigned short ResultSet::add_set(int method_index, int field_index)
{
	// if(!check_method_index_in_range(method_index))
	// {
	// 	WRITE_ERROR("method_index is out of range in ResultSet");
	// 	return RET_FAILURE_INVALID_ARGUMENT;
	// }
	if(!check_field_index_in_range(method_index, field_index, finance_data_type))
	{
...
//	else
//	{
//		for(int i = 1 ; i < FINANCE_DATA_FIELD_AMOUNT_LIST[method_index] ; i++)
...
//	}
	...
	{
		field_index = *iter;
		unsigned short key = get_combined_index(method_index, field_index);
		if (data_set_mapping.find(key) != data_set_mapping.end())
		{
			WRITE_FORMAT_ERROR("The key[%d] from (%d, %d) is duplicate", key, method_index, field_index);
			return RET_FAILURE_INVALID_ARGUMENT;
		}
		unsigned short value;
//		int next_index = -1;
		switch(FINANCE_DATA_FIELD_TYPE_LIST[method_index][field_index])
		{
		case FinanceField_INT:
		{
...
//			}
		...
		case FinanceField_DATE:
			...
			return RET_FAILURE_INVALID_ARGUMENT;
		default:
			WRITE_FORMAT_ERROR("The unsupported field type: %d", FINANCE_DATA_FIELD_TYPE_LIST[method_index][field_index]);
			return RET_FAILURE_INVALID_ARGUMENT;
		}
...
unsigned short ResultSet::add_set(int method_index, const INT_DEQUE& field_set)
{
	unsigned short ret = RET_SUCCESS;
	int field_set_size = field_set.size();
	for (int index = 0 ; index < field_set_size ; index++)
	{
		ret = add_set(method_index, field_set[index]);
		if (CHECK_FAILURE(ret))
			return ret;
	}
	return ret;
...
unsigned short ResultSet::add_calculation_set(int method_index, int field_index, ArrayElementCalculationType calculation_type)
{
	...
	if (calculation_type == ArrayElementCalculation_None)
	{
		...
		return RET_FAILURE_INVALID_ARGUMENT;
//		return add_set(method_index, field_index);
	}
...
// Make sure the array should NOT exist
	unsigned long key_ex = get_combined_index_ex(method_index, field_index, calculation_type);
	if (data_calculation_set_mapping.find(key_ex) != data_calculation_set_mapping.end())
	{
		WRITE_FORMAT_ERROR("The key[%ld] from (%d, %d, %d) is duplicate", key_ex, method_index, field_index, calculation_type);
		return RET_FAILURE_INVALID_ARGUMENT;
	}

// Find the original array
	PFINANCE_DATA_ARRAY_BASE data_array_base = get_array(method_index, field_index);
	if (data_array_base == NULL)
	{
		WRITE_FORMAT_ERROR("The array from (%d, %d) should EXIST", method_index, field_index);
		return RET_FAILURE_INVALID_ARGUMENT;
	}
...
unsigned short ResultSet::find_data_pos(int method_index, int field_index, unsigned short& field_type_index, unsigned short& field_type_pos)const
{
	unsigned short key = get_combined_index(method_index, field_index);
//	if (data_set_mapping.find(key) == data_set_mapping.end())
//	{
//		WRITE_FORMAT_ERROR("The key[%d] from (%d, %d) is NOT FOUND", key, method_index, field_index);
//		return RET_FAILURE_INVALID_ARGUMENT;
//	}
//	unsigned short value = data_set_mapping[key];
	...
	{
		char errmsg[64];
		snprintf(errmsg, 64, "The key[%d] from (%d:%d) is NOT FOUND", key, method_index, field_index);
		WRITE_ERROR(errmsg);
//		throw invalid_argument(errmsg);
		return RET_FAILURE_INVALID_ARGUMENT;
	}
...
unsigned short ResultSet::set_data(int method_index, int field_index, const char* data_string)
{
	if (data_string == NULL)
	{
	...
	ret = find_data_pos(method_index, field_index, field_type_index, field_type_pos);
	if (CHECK_FAILURE(ret))
		return ret;
...
const PFINANCE_##m##_DATA_ARRAY ResultSet::get_##n##_array(int method_index, int field_index)const\
{\
	static const int BUF_SIZE = 64;\
	static char errmsg[BUF_SIZE];\
	unsigned short ret = RET_SUCCESS;\
	...
	if (CHECK_FAILURE(ret))\
	{\
		snprintf(errmsg, BUF_SIZE, "Fail to get data position from (%d:%d)", method_index, field_index);\
		WRITE_ERROR(errmsg);\
		throw invalid_argument(errmsg);\
	}\
...
const PFINANCE_DATA_ARRAY_BASE ResultSet::get_array(int method_index, int field_index)const
{
	int field_type = FINANCE_DATA_FIELD_TYPE_LIST[method_index][field_index];
	switch (field_type)
	{
	case FinanceField_INT:
		return get_int_array(method_index, field_index);
	case FinanceField_LONG:
		return get_long_array(method_index, field_index);
	case FinanceField_FLOAT:
		return get_float_array(method_index, field_index);
	default:
	{
		char errmsg[32];
...
const PFINANCE_DATA_ARRAY_BASE ResultSet::get_array(int method_index, int field_index, ArrayElementCalculationType calculation_type)
{
	static const int BUF_SIZE = 256;
	static char errmsg[BUF_SIZE];

	if (calculation_type == ArrayElementCalculation_None)
		return get_array(method_index, field_index);
...
// If the array should NOT exist, create the array
	if (data_calculation_set_mapping.find(key_ex) == data_calculation_set_mapping.end())
	{
		ret = add_calculation_set(method_index, field_index, calculation_type);
		if (CHECK_FAILURE(ret))
		{
			snprintf(errmsg, BUF_SIZE, "Fail to set array from (%d, %d, %d), due to: %s", method_index, field_index, calculation_type, get_ret_description(ret));
			WRITE_ERROR(errmsg);
...
n ResultSet::get_##n##_array_element(int method_index, int field_index, int index)const\
{\
	PFINANCE_##m##_DATA_ARRAY data_array = get_##n##_array(method_index, field_index);\
	return (*data_array)[index];\
}
...
// Check all the data size are equal
		if (date_data_size != data_size)
		{
			...
			unsigned short method_index = get_upper_subindex(key);
			unsigned short field_index = get_lower_subindex(key);
			WRITE_FORMAT_ERROR("Incorrect data size in %d, %d, expected: %d, actual: %d", method_index, field_index, date_data_size, data_size);
			return RET_FAILURE_INCORRECT_OPERATION;
		}
		iter++;
	}
...
FinanceDataType ResultSetMap::get_data_type()const{return finance_data_type;}
...
{
	assert(result_set != NULL && "result_set should NOT be NULL");
	if (finance_data_type != result_set->get_data_type())
	{
		static const int ERRMSG_SIZE = 256;
		static char errmsg[ERRMSG_SIZE];
		snprintf(errmsg, ERRMSG_SIZE, "The result set of data type[%d] is NOT identical to: %d", result_set->get_data_type(), finance_data_type);
