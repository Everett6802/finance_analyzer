...
// If field_index == -1, it means select all field in the table
	...
	assert(source_field_query_map[source_index] != NULL && "source_field_query_map[source_index] should NOT be NULL");
// Check the index is duplicate
	INT_DEQUE::iterator it = find(source_field_query_map[source_index]->begin(), source_field_query_map[source_index]->end(), 10);
	if (it != source_field_query_map[source_index]->end())
	{
...
// If all fields are selected, it's no need to add extra index
	if (!source_field_query_map[source_index]->empty() && (*source_field_query_map[source_index])[0] == -1)
	{
...
// Add the index
	if (field_index == -1)
		source_field_query_map[source_index]->clear();
	source_field_query_map[source_index]->push_back(field_index);
	return RET_SUCCESS;
}
...
//  Caution: The iterator is const, can NOT be altered
		...
		assert(source_field_query_map[source_type_index] != NULL && "source_field_query_map[source_index] should NOT be NULL");
...
// 	{
// 		assert(source_field_query_map[i] != NULL && "source_field_query_map[source_index] should NOT be NULL");
...
unsigned short ResultSet::add_set(int source_index, const INT_DEQUE& field_set)
{
	unsigned short ret = RET_SUCCESS;
...
unsigned short ResultSet::set_data(int source_index, int field_index, const char* data_string)
{
	if (data_string == NULL)
	{
