...
DatabaseTimeRange* DatabaseTimeRange::instance = NULL;

DatabaseTimeRange* DatabaseTimeRange::get_instance()
{
	if (instance == NULL)
	{
		...
		if (instance == NULL)
		{
			instance = new DatabaseTimeRange();
			if (instance == NULL)
				throw bad_alloc();
			...
			{
				char errmsg[256];
				snprintf(errmsg, 256, "Fail to initialize the DatabaseTimeRange object , due to: %s", get_ret_description(ret));
...
DatabaseTimeRange::DatabaseTimeRange() :
	ref_cnt(0),
...
unsigned short DatabaseTimeRange::initialize()
{
...
void DatabaseTimeRange::deinitialize()
{
...
unsigned short DatabaseTimeRange::get_max_database_time_range(SmartPointer<TimeRangeCfg>& sp_max_database_time_range_cfg)
{
...
int DatabaseTimeRange::add_ref()
{
	__sync_fetch_and_add(&ref_cnt, 1);
	return ref_cnt;
}

int DatabaseTimeRange::release()
{
...
unsigned short DatabaseTimeRange::restrict_time_range(const PQUERY_SET query_set, PTIME_RANGE_CFG time_range_cfg)
{
	if (query_set == NULL)
	{
