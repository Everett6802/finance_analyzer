	...
	statistics(NULL)
{
	...
	if (sql_reader == NULL)
		throw bad_alloc();
	statistics = new FinanceAnalyzerStatistics();
	if (statistics == NULL)
		throw bad_alloc();
...
unsigned short FinanceAnalyzerMgrBase::init_result_set_map()
{
	cleanup_result_set_map();
...
void FinanceAnalyzerMgrBase::cleanup_result_set_map()
{
	if (result_set_map != NULL)
	{
...
// 				}
// 			}
// 			ret = sql_reader->select_data(source_index, string(table_name), field_cmd, (const PDEQUE_INT)&query_field, time_range_cfg_in_year, result_set);
// 			if (CHECK_FAILURE(ret))
// 				return ret;
// 		}

// // Disconnect from the database
// 		ret = sql_reader->disconnect_mysql();
// 		if (CHECK_FAILURE(ret))
// 			return ret;
