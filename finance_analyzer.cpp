...
// PIFINANCE_ANALYZER_MGR create_manager_instance();
...
int main(int argc, char** argv)
{
	...
	PIFINANCE_ANALYZER_MGR finance_analyzer_mgr = NULL;
	...
	for (; index < argc ; index += offset)
	{
        ...
        {
            // param_cfg["mode"] = FinanceAnalysis_Market;
            offset = 1;
        }
        else if (strcmp(argv[index], "--stock_mode") == 0)
        {
            // param_cfg["mode"] = FinanceAnalysis_Stock;
            offset = 1;  
        }
...
// Create the instance of the manager class due to different mode
	finance_analyzer_mgr = g_mgr_factory.get_instance(FinanceAnalysis_Stock);
